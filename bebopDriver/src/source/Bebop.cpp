...
void Bebop::open(ros::NodeHandle & nIn)
{
    //ROS node Handle
    n=nIn;
...
//     {
//         navdataSub = n.subscribe("ardrone/navdata", 1, &Bebop::navdataCallback, this);
...
//             bottomCameraSub = n.subscribe("ardrone/bottom/image_raw", 1, &ARDrone::bottomCameraCallback, this);
    ...
    frontCameraSub = n.subscribe("bebop/image_raw", 1, &Bebop::frontCameraCallback, this);

        commandsSub = n.subscribe("bebop/cmd_vel", 1, &Bebop::commandsCallback, this);
...
//     {
        //Basic commands
        takeOffPubl = n.advertise<std_msgs::Empty>("bebop/takeoff", 1);
        landPubl = n.advertise<std_msgs::Empty>("bebop/land", 1);
        resetPubl = n.advertise<std_msgs::Empty>("bebop/reset", 1);



        //Start chart to publish commands
        commandsPubl = n.advertise<geometry_msgs::Twist>("bebop/cmd_vel", 1);
//     }
...
// void Bebop::navdataCallback(const ardrone_autonomy::Navdata::ConstPtr& msg)
// {
...
void Bebop::commandsCallback(const geometry_msgs::Twist::ConstPtr& msg)
{
...
void Bebop::frontCameraCallback(const sensor_msgs::ImageConstPtr& msg)
{
    //printf("front aqui\n");
...
void Bebop::reset() // AR Drone - emergency stop. It works for both: entering and exiting emergency mode.
{/*
    if(parrotDriverType==parrotDriver::full || parrotDriverType==parrotDriver::drive)
    {
...
void Bebop::setNavCommandToZero()
{
//     if(parrotDriverType==parrotDriver::full || parrotDriverType==parrotDriver::drive)
//     {
        //Set to zero
...
void Bebop::setNavCommand(geometry_msgs::Twist navCommandIn)
{
//     if(parrotDriverType==parrotDriver::full || parrotDriverType==parrotDriver::drive)
//     {
        //Set
...
geometry_msgs::Twist Bebop::getNavCommand(void)
{
...
//JL:
//TODO: ajustar!!!!
void Bebop::setDroneNavCommand(droneMsgs::droneNavCommand navCommandIn)
{
    //Change
...
// //JL:
// //TODO: ajustar!!!!
droneMsgs::droneNavCommand Bebop::getDroneNavCommand(void)
{
    //Change of variables
...
bool Bebop::displayFrontImage(std::string windowName)
{
    if(frontImageRead)
    {
...
bool Bebop::getFrontImage(cv::Mat* frontImageOut, uint32_t *pseq)
{
    if (pseq!=NULL) {
        (*pseq) = frontImage_seq;
    }
...
ros::Time Bebop::getFrontImageTimestamp() {
